...
async function handleCallStarted(callData) {
  ...
  const pool = getDb();
  const { rows } = await pool.query('SELECT * FROM clients WHERE retell_agent_id = $1', [parsed.agentId]);
  const client = rows[0];
  if (!client) return;
...
async function handleCallEnded(callData) {
  ...
  await withClientContext(client.id, async (ctx) => {
    ...
    if (parsed.recordingUrl) {
      ...
      if (recordingPath) {
        await ctx.query('UPDATE calls SET recording_path = $1 WHERE call_id = $2 AND client_id = $3', [recordingPath, parsed.callId, client.id]);
      }
    }
    ...
    if (isMissed && parsed.phoneNumber && !(await isOptedOut(ctx, parsed.phoneNumber, client.id))) {
      await TwilioService.sendFullMenuSMS(parsed.phoneNumber, client.id, ctx);
      if (client.telegram_chat_id && client.ai_enabled) {
